...
Implementing a JSON parser using regular expressions was an interesting and challenging experience. 
One of the challenges was creating accurate regular expressions in tokenize function to match JSON token types properly. 
Another challenge was handling nested arrays and objects. JSON allows nesting of arrays and objects within each other, which requires recursive parsing to handle correctly. I implemented recursive functions within the parser to handle nested structures, ensuring that each level of nesting is processed accurately.
Error handling was also important. I implemented error handling to throw SyntaxError with descriptive messages when encountering unexpected tokens or missing colons in object key-value pairs.